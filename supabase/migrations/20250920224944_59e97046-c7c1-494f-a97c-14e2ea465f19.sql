-- Drop existing conflicting calendar tables to avoid conflicts
...
DROP TYPE IF EXISTS public.session_type_enum CASCADE;
...
-- Now run the updated calendar schema with proper enum casting
-- =========================================================
...
-- 4) Índices
CREATE INDEX idx_sessions_range  ON public.sessions(workspace_id, starts_at, ends_at);
CREATE INDEX idx_sessions_lead   ON public.sessions(lead_id);
CREATE INDEX idx_av_rules_cw     ON public.availability_rules(calendar_id, weekday);
CREATE INDEX idx_av_exceptions_c ON public.availability_exceptions(calendar_id, date);

-- 5) Vistas para la UI
-- 5.1) Vista de sesiones por semana (para WeekGrid)
CREATE VIEW public.view_calendar_week AS
SELECT
  s.id AS session_id,
  s.workspace_id,
  s.calendar_id,
...
CREATE VIEW public.view_quick_availability AS
WITH base AS (
  SELECT
...
SELECT c.workspace_id, c.id, (SELECT id FROM ana), 'S1'::public.session_type_enum, 'scheduled'::public.session_status_enum,
...
FROM cal c
UNION ALL
SELECT c.workspace_id, c.id, (SELECT id FROM car), 'S2'::public.session_type_enum, 'scheduled'::public.session_status_enum,
